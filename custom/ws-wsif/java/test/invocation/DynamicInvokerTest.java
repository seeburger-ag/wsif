...
import clients.DynamicInvoker;

/**
 * Junit test to test out the Dynamic Invoker sample.
...
public class DynamicInvokerTest extends TestCase {
    ...
    public static Test suite() {
        return new TestSuite(DynamicInvokerTest.class);
    }
    ...
    private void doit(String portName, String protocol) {
        ...
        try {
            ...
            float quote = ((Float) (map.get("quote"))).floatValue();
            assertTrue(quote == -1.0F);
        } catch (Exception e) {
            if (portName != null) {
                System.err.println("DynamicInvokerTest(" + portName + ") caught exception " + e);
                e.printStackTrace();
                assertTrue(false);
            } else {
                System.out.println("null port name caused exception as expected");
                assertTrue(true);
            }
        }
    }
