...
public class Conventions {
    ...
    public static String namespaceURI2JavaPath(String namespaceURI)
        ...
        if (namespaceURI == null)
            throw new IllegalArgumentException(
                "Argument to " + "'namespaceURI2JavaPath' cannot " + "be null.");
     ...
     */
    public static String schema2JavaName(
        String schemaType,
        String schemaName,
        boolean isClass)
        throws IllegalArgumentException {
        if (schemaType == null)
            throw new IllegalArgumentException(
                "Illegal arguments to " + "'schema2JavaName'.");

        if (schemaType.compareTo("any") == 0)
            return "any";
        else if (
            schemaType.compareTo("simpleType") == 0) {
            // schemaName should not be null in this case.
            if (schemaName == null)
                throw new IllegalArgumentException(
                    "Illegal arguments to " + "'schema2JavaName'.");
        ...
        schemaName = schemaName.replace('-', '_');
        // some of the id's in Schema is illegal in Java.
    ...
    public static String schema2JavaName(NodeList nl, NodeList targetURI) {
        if (nl.getLength() == 0) {
            throw new IllegalArgumentException(
                "No type name found for serializer " + "class.");
        }
        Node node = nl.item(0);
    ...
    public static String schema2NonQualifiedJavaName(
        NodeList nl,
        NodeList targetURI) {
    ...
    public static String getJavaFileName(
        ...
        String javaFileName =
            schema2NonQualifiedJavaName(nl, targetURI) + javaFileSuffix + ".java";
