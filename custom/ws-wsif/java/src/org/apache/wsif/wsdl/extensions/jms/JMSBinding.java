...
public class JMSBinding implements ExtensibilityElement, Serializable {
    ...
    protected QName fieldElementType = JMSConstants.Q_ELEM_JMS_BINDING;

    // Uses the wrapper type so we can tell if it was set or not.
    protected Boolean fieldRequired = null;

    protected int fieldJmsMessageType = JMSConstants.MESSAGE_TYPE_NOTSET;

    public static String JmsMessageTypeAsString(int type) {
        String str = "NOTSET";

        if (type == JMSConstants.MESSAGE_TYPE_BYTEMESSAGE) {
            str = "ByteMessage";
        } else if (type == JMSConstants.MESSAGE_TYPE_MAPMESSAGE) {
            str = "MapMessage";
        } else if (type == JMSConstants.MESSAGE_TYPE_OBJECTMESSAGE) {
            str = "ObjectMessage";
        } else if (type == JMSConstants.MESSAGE_TYPE_STREAMMESSAGE) {
            str = "StreamMessage";
        } else if (type == JMSConstants.MESSAGE_TYPE_TEXTMESSAGE) {
            str = "TextMessage";
        }
